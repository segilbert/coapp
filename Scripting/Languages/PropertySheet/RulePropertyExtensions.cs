...
namespace CoApp.Toolkit.Scripting.Languages.PropertySheet {
    ...
    public static class RulePropertyExtensions {
        ...
        public static string AsConstrainedString( this IEnumerable<RuleProperty> properties, IEnumerable<string> possibleValues, string defaultValue) {
            var values = properties.SelectMany(each => each.Values);

            if (values.Any()) {
                if (values.Count() > 1) {
                    throw new EndUserPropertyException(properties.First(), "COAPP 100", "Property {0} must only have one value", properties.Name());
                }

                if( properties.First().IsCompoundProperty ) {
                    throw new EndUserPropertyException(properties.First(), "COAPP 103", "Property {0} must not be a compound value",properties.Name());
                }
                ...
                throw new EndUserPropertyException(properties.First(), "COAPP 101", "Property {0} value must be one of {{{1}{2}}}", properties.Name(), possibleValues.Take(10).Aggregate( (result, each) => result + ", " + each), possibleValues.Count() < 10 ? "" : "...");    
            }
            return defaultValue;
        }
        ...
        public static IEnumerable<string> AsStrings( this IEnumerable<RuleProperty> properties) {
         ...
         public static IDictionary<string, IEnumerable<string>> AsDictionary(this IEnumerable<RuleProperty> properties, bool failOnNonCompoundValues = true) {
            ...
            return (from prop in compoundProperties  select prop.LValue).Distinct().ToDictionary(key => key, key => compoundProperties.Where( each => each.LValue == key ).SelectMany( each => each.Values) );
        }
    }
}
