...
//     See http://developer.mindtouch.com/SgmlReader
// </copyright>
//-----------------------------------------------------------------------
...
namespace CoApp.Toolkit.Text.Sgml {
    ...
    internal class HWStack {
        ...
        public object this[int i] {
            get { return (i >= 0 && i < this.m_size) ? m_items[i] : null; }
    ...
    public class SgmlReader : XmlReader {
        ...
        private void SwapTopNodes() {
            var top = this.m_stack.Count - 1;
            if(top > 0) {
                var n = (Node) this.m_stack[top - 1];
                    ...
                    case "":
                        if(NodeType == XmlNodeType.Attribute) {
                            ...
                            for(var i = this.m_stack.Count - 1; i > 0; --i) {
                                var node = this.m_stack[i] as Node;
                    ...
                    default: {
                        ...
                        if(!unknownNamespaces.TryGetValue(prefix, out value)) {
                            if(unknownNamespaces.Count > 0) {
                                value = UNDEFINED_NAMESPACE + unknownNamespaces.Count;
                            }
                            else {
        ...
        public override XmlSpace XmlSpace {
            get {
                for(var i = this.m_stack.Count - 1; i > 1; i--) {
        ...
        public override string GetAttribute(string name) {
            if(this.m_state != State.Attr && this.m_state != State.AttrValue) {
                var i = this.m_node.GetAttribute(name);
                if(i >= 0) {
        ...
        public override string GetAttribute(int i) {
            if(this.m_state != State.Attr && this.m_state != State.AttrValue) {
                var a = this.m_node.GetAttribute(i);
                if(a != null) {
                    return a.Value;
                }
            }
        ...
        private bool ParseTag(char ch) {
            if(ch == '%') {
                ...
                if(ch == '-') {
                    ...
                    var value = this.m_current.ScanToEnd(this.m_sb, "Recovering", ">"); // skip it
                    Log("Ignoring invalid markup '<!" + value + ">");
                    return false;
                }
                else {
                    var name = this.m_current.ScanToken(this.m_sb, declterm, false);
        ...
        private string ScanName(string terminators) {
            var name = this.m_current.ScanToken(this.m_sb, terminators, false);
        ...
        private bool ParseStartTag(char ch) {
            ...
            while(ch != Entity.EOF && ch != '>') {
                ...
                if(ValidAttributeName(aname)) {
                    var a = n.AddAttribute(aname, value ?? aname, quote, this.m_folding == CaseFolding.None);
                    if(a == null) {
        ...
        private bool ParseAspNet() {
            var value = "<%" + this.m_current.ScanToEnd(this.m_sb, "AspNet", "%>") + "%>";
        ...
        private bool ParseComment() {
            ...
            var value = this.m_current.ScanToEnd(this.m_sb, "Comment", "-->");
        ...
        private bool ParseConditionalBlock() {
            var ch = m_current.ReadChar(); // skip '['
            ch = m_current.SkipWhitespace();
            var name = m_current.ScanToken(m_sb, cdataterm, false);
            if(name.StartsWith("if ")) {
                ...
                var value = m_current.ScanToEnd(m_sb, "CDATA", "]]>");
        ...
        private void ParseDocType() {
            ...
            if(ch != '>') {
                ...
                if(ch != '[') {
                    var token = this.m_current.ScanToken(this.m_sb, dtterm, false);
        ...
        private bool ParsePI() {
            var name = this.m_current.ScanToken(this.m_sb, piterm, false);
            string value = null;
        ...
        private bool ParseText(char ch, bool newtext) {
            var ws = !newtext || this.m_current.IsWhitespace;
            if(newtext) {
                this.m_sb.Length = 0;
            }
        ...
        private bool ParseCData() {
            ...
            var ws = this.m_current.IsWhitespace;
            this.m_sb.Length = 0;
            var ch = this.m_current.Lastchar;
            if(this.m_partial != '\0') {
                Pop(); // pop the CDATA
        ...
        private void ExpandEntity(StringBuilder sb, char terminator) {
            var ch = this.m_current.ReadChar();
            if(ch == '#') {
                var charent = this.m_current.ExpandCharEntity();
                sb.Append(charent);
                ch = this.m_current.Lastchar;
            }
            else {
                ...
                if(this.m_dtd != null && !string.IsNullOrEmpty(name)) {
                    ...
                    if(e != null) {
                        if(e.IsInternal) {
                            ...
                            var ex = new Entity(name, e.PublicId, e.Uri, this.m_current.Proxy);
